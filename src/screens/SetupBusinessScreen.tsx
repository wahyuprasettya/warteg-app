...
export const SetupBusinessScreen = () => {
  ...
  const selectedOption = useMemo(
    () => businessOptions.find((option) => option.value === selectedType) ?? businessOptions[0],
    [selectedType],
  );
  ...
  const handleSave = async () => {
    try {
      ...
      Alert.alert("Setup gagal", error.message || "Business type belum bisa disimpan ke Firestore.");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <ScreenContainer scroll>
      <View className="mb-5 overflow-hidden rounded-[36px] border border-brand/15 bg-brand p-6">
        ...
        <View className="w-14 rounded-[20px] bg-white/15 px-3 py-3">
          <Text className="text-center text-2xl">{businessDecor[selectedType].icon}</Text>
        </View>
        ...
        <Text className="mt-3 text-sm leading-6 text-white/80">
          Kami sesuaikan alur kasir, kategori, dan navigasi agar terasa paling cepat untuk model usaha kamu.
        </Text>
        ...
        <View className="mt-5 rounded-[26px] bg-white/12 p-4">
          <View className="flex-row items-center justify-between">
            <View className="flex-1 pr-3">
              ...
              <Text className="mt-1 text-2xl font-poppins-bold text-white">{selectedOption.label}</Text>
            </View>
            <View className="rounded-full bg-white/15 px-3 py-2">
              <Text className="text-xs font-poppins-semibold text-white">{businessDecor[selectedType].badge}</Text>
            </View>
          </View>
      ...
      <View className="mb-4">
        <Text className="text-lg font-poppins-bold text-brand-ink">Pilih yang paling cocok</Text>
        <Text className="mt-1 text-sm text-brand-muted">
          Kamu bisa pilih mode yang paling sesuai dengan cara jualan sehari-hari.
        </Text>
      </View>
          ...
          return (
            <Pressable
              ...
              className={`mb-3 overflow-hidden rounded-[30px] border p-5 ${
                isActive ? "border-brand bg-white" : "border-brand/5 bg-white/80"
              }`}
              ...
              <View className="flex-row items-start justify-between">
                <View className="mr-3 flex-1">
                  <View className="mb-3 flex-row items-center">
                    <View className={`mr-3 h-12 w-12 items-center justify-center rounded-[18px] ${isActive ? "bg-brand" : "bg-brand-soft"}`}>
                      <Text className="text-2xl">{decor.icon}</Text>
                    </View>
                    <View className="flex-1">
                      <Text className="text-xl font-poppins-bold text-brand-ink">{option.label}</Text>
                  ...
                  <Text className="text-sm leading-6 text-brand-muted">{option.description}</Text>

                  <View className="mt-4 flex-row flex-wrap">
                    {decor.perks.map((perk) => (
                      <View key={perk} className="mr-2 mb-2 rounded-full bg-brand-soft/70 px-3 py-1.5">
                        <Text className="text-[11px] font-poppins-semibold text-brand">{perk}</Text>
                      </View>
                    ))}
                  </View>
                </View>

                <View className={`h-8 w-8 items-center justify-center rounded-full ${isActive ? "bg-brand" : "bg-brand-soft"}`}>
                  <Text className={`text-sm font-poppins-bold ${isActive ? "text-white" : "text-brand-muted"}`}>
                    {isActive ? "✓" : ""}
                  </Text>
                </View>
              </View>
      ...
      <View className="rounded-[28px] border border-brand/5 bg-white p-4">
        ...
        <Text className="mt-2 text-xl font-poppins-bold text-brand-ink">{selectedOption.label}</Text>
        <Text className="mt-1 text-sm leading-5 text-brand-muted">{selectedOption.description}</Text>

        <View className="mt-4">
          <AppButton label="Simpan & Mulai Jualan" onPress={handleSave} loading={isSaving} />
        </View>
      </View>
    </ScreenContainer>
  );
