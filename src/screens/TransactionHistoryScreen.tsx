  ...
  return (
    <ScreenContainer>
      <AppHeader
        ...
        subtitle="Pantau transaksi yang baru masuk."
      ...
      {transactions.length === 0 ? (
        <EmptyState
          ...
          description="Riwayat akan muncul otomatis setelah checkout."
        />
      ) : (
        <ScrollView showsVerticalScrollIndicator={false}>
          {transactions.map((transaction) => (
            <View
              key={transaction.id}
              className="mb-3 rounded-[28px] bg-white p-4"
            >
              <View className="flex-row items-start justify-between">
                <View className="flex-1 pr-3">
                  <Text className="text-lg font-poppins-bold text-brand-ink">
                    {friendlyDateTime(transaction.createdAt)}
                  </Text>
              ...
              <View className="mt-3">
                {transaction.items.map((item) => (
                  <Text
                    className="font-poppins mb-1 text-base text-brand-ink"
